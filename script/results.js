document.addEventListener("DOMContentLoaded", () => {
    ...
    if (resultData) {
        document.getElementById("total-score").textContent = `${Math.round(resultData.score)}%`;
        document.getElementById("correct-count").textContent = resultData.correct;
        document.getElementById("incorrect-count").textContent = resultData.incorrect;
        document.getElementById("time-taken").textContent = resultData.time;
        document.querySelector(".summary-item:nth-child(3) h3").textContent = "Time Spent";
        document.getElementById("time-taken").textContent = resultData.time;
    } else {
